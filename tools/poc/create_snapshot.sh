...
dmsetup remove fc-dev-thinpool-snap-$THINPOOL_DEVICE_NAME || true
dmsetup message /dev/mapper/fc-dev-thinpool 0 "delete $THINPOOL_TABLE_NUM" || true

# Create snapshot
firecracker-ctr --address /run/firecracker-containerd/containerd.sock images pull --snapshotter devmapper $IMAGE
mv $SNAPSHOT_PATH $SNAPSHOT_PATH.1 || true
...
dmsetup message /dev/mapper/fc-dev-thinpool 0 "create_snap $THINPOOL_TABLE_NUM $TABLE_NUMBER"
dmsetup resume $DEVICE
dmsetup create fc-dev-thinpool-snap-$THINPOOL_DEVICE_NAME --table "$TABLE_PART $THINPOOL_TABLE_NUM"

# Recofig snapshot file and move to snapshot folder
sed -i "s/fc-dev-thinpool-snap-[0-9][0-9][0-9]/fc-dev-thinpool-snap-$THINPOOL_DEVICE_NAME/g" snapshot_file
cp mem_file $SNAPSHOT_PATH.1/
cp snapshot_file $SNAPSHOT_PATH.1/
popd

mv $SNAPSHOT_PATH.1 $SNAPSHOT_PATH || true
